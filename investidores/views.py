...
from empresarios.models import Empresa, Documento, Metrica
...
def sugestao(request):
    ...
    if request.method == 'GET':
        ...
        if tipo == 'C':
            empresas = Empresa.objects.filter(tempo_existencia='+5').filter(estagio='E')
            empresas = empresas.filter(area__in=area)
        elif tipo == 'D':
            empresas = Empresa.objects.filter(tempo_existencia__in=['-6', '+6', '+1']).exclude(estagio='E')
            empresas = empresas.filter(area__in=area)
        elif tipo == 'G':
...
def realizar_proposta(request, id):
    ...
    if valuation < (int(empresa.valuation / 2)):
        messages.add_message(request, constants.WARNING, f'Seu valuation proposta foi R${valuation} e deve ser no mínimo {empresa.valuation}')
        return redirect(f'/investidores/ver_empresa/{id}')
